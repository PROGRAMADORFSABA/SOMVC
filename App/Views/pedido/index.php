                        ...
                        <td>
                            <span class="dropdown">
                                <a href="#" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="dropdown"
                                    title="click aqui para exibir as acoes" aria-expanded="true"><i
                                        class="la la-ellipsis-h"></i></a>
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a class="dropdown-item"
                                        href="http://<?php echo APP_HOST; ?>/pedido/edicao/<?php echo $pedido->getCodControle(); ?>"
                                        title="Alterar pedido" class="btn btn-info btn-sm"><i class="la la-edit"></i>
                                        Alterar</a>
                                    <a class="dropdown-item"
                                        href="http://<?php echo APP_HOST; ?>/pedido/exclusao/<?php echo $pedido->getCodControle(); ?>"
                                        title="Excluir" class="btn btn-info btn-sm"><i class="la la-trash"></i>
                                        Excluir</a>
                                    <a class="dropdown-item" data-codstatus="<?php echo $pedido->getCodControle(); ?>"
                                        data-toggle="modal" data-target="#kt_select2_modal" title="Alterar Status"
                                        class="btn btn-info btn-sm"><i class="la la-leaf"></i> Status</a>
                                    <a class="dropdown-item"
                                        href="http://<?php echo APP_HOST; ?>/pedido/visualisar/<?php echo $pedido->getCodControle(); ?>"
                                        title="Visualisar" class="btn btn-info btn-sm"><i class="la la-edit"></i>
                                        Visualisar</a>
                                    <a class="dropdown-item"
                                        href="http://<?php echo APP_HOST; ?>/pedido/edicao/<?php echo $pedido->getCodControle(); ?>"
                                        ...
                                        Relatorio</a>
                                    <a class="dropdown-item"
                                        href="http://<?php echo APP_HOST; ?>/public/assets/media/anexos/<?php echo $pedido->getAnexo(); ?>"
                            ...
                            <a href="http://<?php echo APP_HOST; ?>/pedido/visualisar/<?php echo $pedido->getCodControle(); ?>"
                                title="Visualisar" class="btn btn-clean btn-icon btn-icon-md"><i class="la la-leaf"></i>
                            </a>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="">Alteracao de Status - <b>
                        <?php echo $pedido->getClienteLicitacao()->getRazaoSocial(); ?></b></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true" class="la la-remove"></span>
                </button>
            </div>
            <form class="kt-form kt-form--fit kt-form--label-right"
                action="http://<?php echo APP_HOST; ?>/pedido/atualizar" method="post" id="form_cadastro">
                <input type="text" class="form-control" name="codControle" id="codControle"
                    value="<?php echo $pedido->getCodControle(); ?>" required>
                <input type="text" class="form-control" name="cliente" id="cliente"
                    value="<?php echo $pedido->getClienteLicitacao()->getCodCliente(); ?>" required>
                <input type="text" class="form-control" name="usuario" id="usuario"
                    value="<?php echo $_SESSION['id']; ?>" required>
                <input type="text" class="form-control" name="fk_instituicao" id="fk_instituicao"
                    ...
                    <div class="form-group row kt-margin-t-20">
                        ...
                        <div class="col-lg-9 col-md-9 col-sm-12">
                            <select class="form-control m-select2" id="kt_select2_1_modal" name="status">
                                ...
                                <?php foreach ($viewVar['listaStatus'] as $status) : ?>
                                <option value="<?php echo $status->getCodStatus(); ?>"
                                    <?php echo ($Sessao::retornaValorFormulario('status') == $status->getCodStatus()) ? "selected" : ""; ?>>
                                    <?php echo $status->getNome(); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
